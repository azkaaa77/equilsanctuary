...
const dict = {
  id: {
    ...
    headline2: "Sosialmu.",
    subtitle:
      "Peta relasi dan manajemen baterai sosial untuk menjaga kejernihan hubungan.",
    ...
    battery: {
      ...
      subtitle:
        "Seberapa banyak energi sosial yang kamu miliki? Catat secara jujur setiap hari.",
    ...
    bonds: {
      ...
      subtitle:
        "Katalog orang-orang dalam hidupmu — siapa yang mengisi dan siapa yang menguras energimu.",
      ...
      add: "TAMBAH",
      empty:
        "Belum ada hubungan tercatat. Mulai petakan orang-orang di sekitarmu.",
      delete: "hapus",
    },
    ...
    map: {
      ...
      subtitle:
        "Gambaran visual tentang ekosistem relasi dan keseimbangan energimu.",
      ...
      balancePoor:
        "Hati-hati — lebih banyak penguras daripada pengisi energimu.",
    ...
    rituals: {
      title: "Ritual Sosial",
      subtitle:
        "Kebiasaan dan ritme sosial yang secara aktif mengisi ulang baterai jiwamu.",
      addPlaceholder:
        "Nama ritual (cth: Jalan pagi sendiri, Makan malam keluarga)",
      ...
      defaultRituals: [
        {
          title: "Waktu sendiri tanpa gangguan",
          frequency: "Setiap hari",
          energyGain: 5,
        },
        {
          title: "Percakapan mendalam dengan sahabat",
          frequency: "Seminggu sekali",
          energyGain: 4,
        },
        {
          title: "Digital detox selama 1 jam",
          frequency: "Setiap hari",
          energyGain: 4,
        },
        {
  ...
  en: {
    ...
    headline2: "Energy.",
    subtitle:
      "Relationship mapping and social battery management for relationship clarity.",
    statusLabel: "SOCIAL ENERGY MAPPED",
    tabs: {
      battery: "BATTERY",
      bonds: "BONDS",
    ...
    battery: {
      ...
      subtitle:
        "How much social energy do you have? Log it honestly every day.",
      logLabel: "Social activities today",
      logPlaceholder:
        "What social interactions did you have today? (meetings, hangouts, calls...)",
    ...
    bonds: {
      ...
      subtitle:
        "The people in your life — who fills and who drains your energy.",
      addTitle: "Add a Person",
      namePlaceholder: "Name or initials",
      notesPlaceholder: "Notes about this person (optional)",
      tagsPlaceholder:
        "Tags: separate with commas (e.g., supportive, introvert)",
      categories: {
        family: "Family",
        friend: "Friend",
        colleague: "Colleague",
    ...
    map: {
      ...
      subtitle:
        "A visual overview of your relationship ecosystem and energy balance.",
    ...
    rituals: {
      ...
      subtitle:
        "Social habits and rhythms that actively recharge your soul battery.",
      ...
      defaultRituals: [
        {
          title: "Uninterrupted alone time",
          frequency: "Every day",
          energyGain: 5,
        },
        {
          title: "Deep conversation with a close friend",
...
export default function SoulCommunityPage() {
  ...
  const [editBatteryActivities, setEditBatteryActivities] = useState("");
  const [editBatteryNote, setEditBatteryNote] = useState("");
  const [editBatteryLevel, setEditBatteryLevel] = useState<BatteryLevel>(5);
  ...
  const [editBondName, setEditBondName] = useState("");
  const [editBondCategory, setEditBondCategory] =
    useState<RelationCategory>("friend");
  const [editBondEnergyType, setEditBondEnergyType] =
    useState<EnergyType>("neutral");
  const [editBondScore, setEditBondScore] = useState(0);
  const [editBondLastSeen, setEditBondLastSeen] = useState("");
  const [editBondNotes, setEditBondNotes] = useState("");
  const [editBondTags, setEditBondTags] = useState("");
  ...
  const [editRitualFreq, setEditRitualFreq] = useState("");
  const [editRitualEnergy, setEditRitualEnergy] = useState(3);
  ...
  // Persist load
  useEffect(() => {
    const bl = localStorage.getItem("sc_battery");
    if (bl) setBatteryLogs(JSON.parse(bl));
    const bd = localStorage.getItem("sc_bonds");
    if (bd) setBonds(JSON.parse(bd));
  ...
  const deleteBatteryLog = (id: string) => {
    const updated = batteryLogs.filter((log) => log.id !== id);
    setBatteryLogs(updated);
    localStorage.setItem("sc_battery", JSON.stringify(updated));
  };

  const saveEditBatteryLog = (id: string) => {
    const updated = batteryLogs.map((log) =>
      log.id === id
        ? {
            ...log,
            activities: editBatteryActivities,
            note: editBatteryNote,
            level: editBatteryLevel,
          }
        : log,
    );
    setBatteryLogs(updated);
  ...
  const addBond = () => {
    ...
    setBondTags("");
    setBondScore(0);
    setBondEnergyType("neutral");
    setBondCategory("friend");
    setBondLastSeen("");
    setAddingBond(false);
  };
  ...
  const saveEditBond = (id: string) => {
    if (!editBondName.trim()) return;
    const updated = bonds.map((b) =>
      b.id === id
        ? {
            ...b,
            name: editBondName,
            category: editBondCategory,
            energyType: editBondEnergyType,
            energyScore: editBondScore,
            lastInteraction:
              editBondLastSeen || new Date().toISOString().split("T")[0],
            notes: editBondNotes,
            tags: editBondTags
              .split(",")
              .map((t) => t.trim())
  ...
  const toggleRitualDone = (id: string) => {
    ...
    localStorage.setItem(
      "sc_today_rituals",
      JSON.stringify({ date: new Date().toDateString(), ids: updated }),
    );
  ...
  const deleteRitual = (id: string) => {
    ...
    setRituals(updated);
    localStorage.setItem("sc_rituals", JSON.stringify(updated));
    if (todayDoneRituals.includes(id)) {
      const updatedDone = todayDoneRituals.filter((r) => r !== id);
      setTodayDoneRituals(updatedDone);
      localStorage.setItem(
        "sc_today_rituals",
        JSON.stringify({ date: new Date().toDateString(), ids: updatedDone }),
      );
    }
  };

  const startEditRitual = (ritual: SocialRitual) => {
    setEditingRitualId(ritual.id);
    setEditRitualTitle(ritual.title);
    setEditRitualFreq(ritual.frequency);
    setEditRitualEnergy(ritual.energyGain);
  };

  const saveEditRitual = () => {
    if (!editRitualTitle.trim()) return;
    const updated = rituals.map((r) =>
      r.id === editingRitualId
        ? {
            ...r,
            title: editRitualTitle,
            frequency: editRitualFreq,
            energyGain: editRitualEnergy,
          }
        : r,
    );
    setRituals(updated);
  ...
  const topDrainer = [...drainers].sort(
  ...
  const avgBattery =
    batteryLogs.length > 0
      ? Math.round(
          batteryLogs.slice(0, 7).reduce((s, l) => s + l.level, 0) /
            Math.min(7, batteryLogs.length),
        )
      : null;
    ...
    <div className="space-y-12 pb-24">
      ...
      <section className="relative w-full">
        <div
          ...
          style={{
            background:
              "radial-gradient(circle, rgba(45,106,79,0.035) 0%, transparent 70%)",
        ...
        >
          <h1 className="text-6xl md:text-[6rem] leading-[1] font-black text-slate-900 tracking-tight">
            Your Social
            <br />
            <span className="font-serif italic text-emerald-700">Energy.</span>
          </h1>
        </div>
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          <span className="text-[8px] font-mono tracking-[0.35em] text-[#141313]/25 uppercase">
            {t.statusLabel}
          </span>
          ...
          <div>
                    ...
                    style={{
                      ...
                      backgroundColor:
                        batteryLevel >= lv
                          ...
                          : "rgba(20,19,19,0.06)",
                      opacity:
                        batteryLevel === lv ? 1 : batteryLevel > lv ? 0.5 : 0.3,
                    }}
                  />
                  <span
            ...
            <div className="flex items-center gap-3 mb-6">
              <div
                ...
                style={{
                  backgroundColor: BATTERY_COLORS[batteryLevel - 1],
                  opacity: 0.7,
                }}
              />
              ...
              <span className="text-[9px] font-mono text-[#141313]/20">
                {batteryLevel}/10
              </span>
            </div>
          </div>
          ...
          {batteryLogs.length > 0 && (
            <div className="flex gap-8 pt-2 border-t border-black/5">
              {avgBattery !== null && (
                <div>
                  <span
                    className="block text-3xl font-extrabold tracking-tighter leading-none mb-1"
                    style={{
                      color: BATTERY_COLORS[avgBattery - 1],
                      opacity: 0.4,
                    }}
                  >
                    {avgBattery}
            ...
            {batteryLogs.length === 0 ? (
              ...
              <div className="space-y-3">
                {batteryLogs.slice(0, 14).map((log) => (
                  <div
                    key={log.id}
                    className="group flex flex-row items-center justify-between p-4 bg-white rounded-2xl shadow-sm border border-gray-100 mb-3"
                  >
                    {editingBatteryId === log.id ? (
                      <div className="flex-1 space-y-3">
                        <div className="flex flex-col gap-2">
                          <textarea
                            value={editBatteryActivities}
                            onChange={(e) =>
                              setEditBatteryActivities(e.target.value)
                            }
                          ...
                          <input
                            type="text"
                            value={editBatteryNote}
                            onChange={(e) => setEditBatteryNote(e.target.value)}
                            className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-3 py-2 text-xs text-[#141313]/80 font-sans outline-none focus:border-emerald-400"
                            placeholder={t.battery.notePlaceholder}
                          />
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-[8px] font-mono text-[#141313]/40 tracking-wider uppercase">
                              Level:
                            </span>
                            <div className="flex gap-1 flex-wrap">
                              {(
                                ...
                                <button
                                  key={lv}
                                  onClick={() => setEditBatteryLevel(lv)}
                                  className="w-5 h-5 rounded flex items-center justify-center text-[9px] font-mono transition-all duration-200"
                                  style={{
                                    backgroundColor:
                                      editBatteryLevel === lv
                                        ? BATTERY_COLORS[lv - 1]
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => saveEditBatteryLog(log.id)}
                            className="px-4 py-1.5 rounded-full bg-emerald-700 text-white text-[8px] font-mono font-bold tracking-widest uppercase hover:bg-emerald-800 transition-colors"
                          >
                            {language === "id" ? "SIMPAN" : "SAVE"}
                          </button>
                          <button
                            onClick={() => setEditingBatteryId(null)}
                            className="px-4 py-1.5 rounded-full border border-gray-200 text-gray-500 text-[8px] font-mono font-bold tracking-widest uppercase hover:bg-gray-50 transition-colors"
                          >
                      ...
                      <>
                        ...
                        <div className="flex-1 pr-4 flex items-start gap-4">
                          <div
                            ...
                            style={{
                              backgroundColor:
                                BATTERY_COLORS[log.level - 1] + "20",
                            }}
                          >
                            <span
                              className="text-[9px] font-mono font-bold"
                              style={{ color: BATTERY_COLORS[log.level - 1] }}
                            >
                              {log.level}
                            </span>
                          </div>
                          <div className="min-w-0 flex-1">
                            {log.activities && (
                              <p className="text-[10px] font-sans text-[#141313]/60 mb-0.5">
                                {log.activities}
                              </p>
                            )}
                            {log.note && (
                              <p className="text-[9px] font-sans italic text-[#141313]/30">
                                "{log.note}"
                              </p>
                            )}
                            <span className="text-[7px] font-mono text-[#141313]/20 tracking-widest mt-1 block">
                              {log.date}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 shrink-0 m-0">
                          ...
                          <button
                            onClick={() => {
                              ...
                              setEditBatteryActivities(log.activities || "");
      ...
      {activeTab === "bonds" && (
        <section className="space-y-8">
          ...
          <button
            ...
            className={
              addingBond
                ? "px-8 py-3 rounded-full text-[9px] font-mono font-bold tracking-[0.35em] uppercase transition-all duration-200 border border-black/15 text-[#141313]/40 hover:text-[#141313]"
          ...
          {addingBond && (
            <div className="space-y-5 border-t border-black/6 pt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {(Object.keys(CATEGORY_ICONS) as RelationCategory[]).map(
                  (cat) => (
                    <button
                      key={cat}
                    ...
                    >
                      ...
                      {
                        (
                          t.bonds.categories as Record<RelationCategory, string>
                        )[cat]
                      }
                    </button>
                  ),
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {(["giver", "neutral", "drainer"] as EnergyType[]).map(
                    (type) => (
                      <button
                        key={type}
                        ...
                        style={
                          bondEnergyType === type
                            ? {
                                backgroundColor: ENERGY_CONFIG[type].dot + "CC",
                              }
                            : {}
                        }
                      >
                        {language === "id"
                          ? ENERGY_CONFIG[type].labelId
                          : ENERGY_CONFIG[type].labelEn}
                      </button>
                    ),
                  )}
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-[8px] font-mono text-[#FF8A80]/60">
                    {t.bonds.scoreMinus}
                  </span>
                  ...
                  <span className="text-[8px] font-mono text-[#2D6A4F]/60">
                    {t.bonds.scorePlus}
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-0">
              {bonds.map((bond) => {
                ...
                return (
                  <div
                    key={bond.id}
                    className="group bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-row items-center justify-between p-4 mb-3"
                  >
                    {editingBondId === bond.id ? (
                      <div className="flex-1 space-y-3">
                        <input
                          value={editBondName}
                          onChange={(e) => setEditBondName(e.target.value)}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(
                            Object.keys(CATEGORY_ICONS) as RelationCategory[]
                          ).map((cat) => (
                            <button
                              key={cat}
                              onClick={() => setEditBondCategory(cat)}
                              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[8px] font-mono tracking-widest uppercase transition-all duration-200 ${
                                editBondCategory === cat
                                  ? "bg-emerald-700 text-white shadow-sm"
                                  : "bg-white border border-gray-200 shadow-sm text-gray-600 hover:bg-gray-50"
                              }`}
                            >
                              {CATEGORY_ICONS[cat]}{" "}
                              {
                        ...
                        <div className="flex gap-2">
                          {(
                            ["giver", "neutral", "drainer"] as EnergyType[]
                          ).map((type) => (
                            <button
                              key={type}
                              onClick={() => setEditBondEnergyType(type)}
                              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[8px] font-mono tracking-widest uppercase border transition-all duration-200 ${
                                editBondEnergyType === type
                                  ? type === "giver"
                                    ? "bg-emerald-600 border-emerald-600 text-white"
                                    : type === "drainer"
                                      ? "bg-rose-400 border-rose-400 text-white"
                                      : "bg-slate-400 border-slate-400 text-white"
                                  : "bg-white border-gray-200 shadow-sm text-gray-600 hover:bg-gray-50"
                              }`}
                            >
                              {language === "id"
                                ? ENERGY_CONFIG[type].labelId
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-[8px] font-mono text-[#141313]/50">
                            Score (-5 to 5):
                          </span>
                          <input
                            type="number"
                            ...
                            value={editBondScore}
                            onChange={(e) =>
                              setEditBondScore(Number(e.target.value))
                            }
                            className="w-16 bg-white border border-gray-200 shadow-sm rounded-xl px-2 py-1 text-xs text-[#141313]/80 font-sans outline-none focus:border-emerald-400"
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            type="date"
                            value={editBondLastSeen}
                            onChange={(e) =>
                              setEditBondLastSeen(e.target.value)
                            }
                            className="bg-white border border-gray-200 shadow-sm rounded-xl px-2 py-1.5 text-[9px] font-mono text-[#141313]/60 focus:border-slate-400 outline-none"
                          />
                          <input
                            type="text"
                            value={editBondTags}
                            onChange={(e) => setEditBondTags(e.target.value)}
                            className="bg-white border border-gray-200 shadow-sm rounded-xl px-2 py-1.5 text-xs text-[#141313]/60 placeholder:text-[#141313]/40 font-sans focus:border-slate-400 outline-none"
                            placeholder={t.bonds.tagsPlaceholder}
                          />
                        </div>
                        <textarea
                          value={editBondNotes}
                          onChange={(e) => setEditBondNotes(e.target.value)}
                          rows={2}
                          className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-3 py-2 text-xs text-[#141313]/60 placeholder:text-[#141313]/40 font-sans focus:border-slate-400 outline-none"
                          placeholder={t.bonds.notesPlaceholder}
                        />
                        <div className="flex gap-2">
                          <button
                            onClick={() => saveEditBond(bond.id)}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-3 mb-1.5">
                              <span
                                ...
                                style={{ color: energyCfg.dot, opacity: 0.9 }}
                              >
                                {language === "id"
                                  ? energyCfg.labelId
                                  : energyCfg.labelEn}
                            ...
                            {bond.tags.length > 0 && (
                              <div className="flex flex-wrap gap-1.5 mb-1.5">
                                {bond.tags.map((tag, i) => (
                                  <span
                                    key={i}
                                    className="text-[7px] font-mono text-[#141313]/50 border border-gray-200 bg-gray-50 px-2 py-0.5 rounded-full"
                                  >
                                    {tag}
                                  </span>
                                ))}
                            ...
                            {bond.notes && (
                              <p className="text-[9px] font-sans italic text-[#141313]/40">
                                "{bond.notes}"
                              </p>
                            )}
                          </div>
            ...
            <>
              ...
              <div className="flex gap-10 flex-wrap border-b border-black/6 pb-8">
                {[
                  {
                    value: `+${totalGain}`,
                    label: t.map.totalGain,
                    color: "#2D6A4F",
                  },
                  {
                    value: `-${totalDrain}`,
                    label: t.map.totalDrain,
                    color: "#FF8A80",
                  },
                  {
              ...
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{
                    backgroundColor: netEnergy >= 0 ? "#2D6A4F" : "#FF8A80",
                    opacity: 0.6,
                  }}
                />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[8px] font-mono text-[#141313]/20">
                      ({givers.length})
                    </span>
                  </div>
                  {givers.length === 0 ? (
                    <p className="text-[9px] font-mono text-[#141313]/15 italic">
                      —
                    </p>
                  ) : (
                          ...
                          <div key={b.id} className="flex items-center gap-3">
                            <div className="flex-1 h-[2px] bg-black/5 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${(b.energyScore / 5) * 100}%`,
                                  backgroundColor: "#2D6A4F",
                                  opacity: 0.4,
                                }}
                              />
                            </div>
                            <span className="text-[10px] font-sans text-[#141313]/60 min-w-0 truncate">
                              {b.name}
                            </span>
                          </div>
                        ))}
                    ...
                    <div className="space-y-2">
                      {neutrals.map((b) => (
                        <div key={b.id} className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-sans text-[#141313]/40 truncate">
                            {b.name}
                          </span>
                        </div>
                      ))}
                          ...
                          <div key={b.id} className="flex items-center gap-3">
                            <div className="flex-1 h-[2px] bg-black/5 rounded-full overflow-hidden">
                              <div
                                className="h-full rounded-full"
                                style={{
                                  width: `${(Math.abs(b.energyScore) / 5) * 100}%`,
              ...
              {(topGiver || topDrainer) && (
                <div className="flex gap-10 pt-2 border-t border-black/5">
                  {topGiver && (
                    <div>
                      <span className="text-[7px] font-mono tracking-widest text-[#141313]/20 uppercase block mb-1">
                        {t.map.topGiver}
                      </span>
                      <span className="text-sm font-sans text-[#2D6A4F]/60">
                        {topGiver.name}
                      </span>
                    </div>
                  )}
                  {topDrainer && (
                    <div>
                      ...
                      <span className="text-sm font-sans text-[#FF8A80]/60">
                        {topDrainer.name}
                      </span>
                    </div>
                  )}
            ...
            <div className="flex items-center gap-2 pb-2">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      newRitualEnergy >= n ? "#2D6A4F" : "rgba(20,19,19,0.07)",
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1 h-[2px] bg-black/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(todayDoneRituals.length / rituals.length) * 100}%`,
                  }}
                />
              </div>
              <span className="text-[8px] font-mono text-[#141313]/25 shrink-0">
                {todayDoneRituals.length}/{rituals.length}{" "}
            ...
            <div className="space-y-0">
              {rituals.map((ritual) => {
                ...
                return (
                  <div
                    key={ritual.id}
                    className="group bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-row items-center justify-between p-4 mb-3 min-h-[72px]"
                  >
                    {editingRitualId === ritual.id ? (
                      <div className="flex-1 space-y-3">
                        <div className="flex flex-col gap-2">
                          <input
                            ...
                            className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-3 py-2 text-xs text-[#141313]/80 font-sans outline-none focus:border-slate-400"
                            placeholder={t.rituals.addPlaceholder}
                          />
                          <div className="flex flex-wrap items-center gap-4">
                            <input
                              type="text"
                              value={editRitualFreq}
                              onChange={(e) =>
                                setEditRitualFreq(e.target.value)
                              }
                              className="bg-white border border-gray-200 shadow-sm rounded-xl px-3 py-1.5 text-[9px] font-mono text-[#141313]/60 focus:border-slate-400 outline-none w-36"
                              placeholder={t.rituals.freqPlaceholder}
                            />
                                    ...
                                    style={{
                                      backgroundColor:
                                        editRitualEnergy >= n
                                          ? "#2D6A4F"
                                          : "rgba(20,19,19,0.07)",
                                      opacity: editRitualEnergy >= n ? 0.6 : 1,
                                    }}
                                  />
                                ))}
                              </div>
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={saveEditRitual}
                            className="px-4 py-1.5 rounded-full bg-slate-800 text-white text-[8px] font-mono font-bold tracking-widest uppercase hover:bg-slate-700 transition-colors"
                          >
                            {language === "id" ? "SIMPAN" : "SAVE"}
                          </button>
                          <button
                      ...
                      <>
                        <div className="flex-1 pr-4 flex items-start gap-4">
                          ...
                          <div className="min-w-0 flex-1">
                            <p
                              className={`text-[11px] font-sans transition-all duration-200 mb-1 ${
                                done
                                  ? "line-through text-[#141313]/30"
                                  : "text-[#141313]/70"
                              }`}
                            >
                              {ritual.title}
                                    ...
                                    style={{
                                      backgroundColor:
                                        ritual.energyGain >= n
                                          ? "#2D6A4F"
                                          : "rgba(20,19,19,0.07)",
                                      opacity: ritual.energyGain >= n ? 0.6 : 1,
                                    }}
                                  />
                                ))}
                              </div>
