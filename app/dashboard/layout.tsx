...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    const hasAuthFragment = typeof window !== 'undefined' && window.location.hash.includes('access_token');
    ...
    return (
      <div className="min-h-screen bg-[#FFFFFF] text-[#141313] flex items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <span className="text-[10px] tracking-[0.3em] font-mono text-[#141313]/40 uppercase">{t.systemInit}</span>
        </div>
      </div>
    );
  }
  ...
  const moduleNav = [
    {
      ...
      icon: '◌',
      desc: language === 'id' ? 'Jurnal & Zen Tracker' : 'Journal & Zen Tracker',
      disabled: false,
    },
    {
      ...
      icon: '◆',
      desc: language === 'id' ? 'Karier & Sertifikasi' : 'Career & Certifications',
      disabled: false,
    },
    {
      ...
      icon: '◎',
      desc: language === 'id' ? 'Vault & Pertahanan Finansial' : 'Vault & Financial Defense',
      disabled: false,
    },
    {
      label: 'SANCTUARY AI',
      ...
      icon: '◉',
      desc: language === 'id' ? 'Refleksi Emosional 24/7' : 'Emotional Reflection 24/7',
      disabled: false,
    },
    {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-equil-paper text-equil-onyx relative font-sans">
      ...
      <div className="flex items-center justify-between p-4 border-b border-equil-forest/10 md:hidden bg-equil-paper sticky top-0 z-40">
        <a href="/" className="text-lg font-black tracking-tighter text-[#141313]">
          EQUIL.
        </a>
        <button
      ...
      {/* ── SIDEBAR (RESPONSIVE) ── */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-equil-paper border-r border-equil-forest/10 transform transition-transform duration-300 ease-in-out md:sticky md:top-0 md:h-screen md:translate-x-0 flex flex-col overflow-y-auto custom-scrollbar shrink-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="px-6 pt-10 pb-6 border-b border-black/5 shrink-0 flex items-center justify-between">
          <div>
            <a href="/" className="text-xl font-black tracking-tighter text-[#141313] hover:text-[#2D6A4F] transition-colors">
              EQUIL.
            </a>
          ...
          <div className="flex flex-col gap-0.5">
            {mainNav.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm transition-colors ${active ? 'text-[#F9F9F9]/50' : 'text-[#141313]/20 group-hover:text-[#141313]/40'}`}>
                    {item.icon}
                  </span>
                  {item.label}
                </a>
          ...
          <div className="flex flex-col gap-0.5">
            {moduleNav.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm mt-0.5 shrink-0 transition-colors ${
                    active ? 'text-[#F9F9F9]/50' : 'text-[#141313]/20 group-hover:text-[#141313]/40'
                  }`}>
                    {item.icon}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className={`text-[10px] font-mono font-bold tracking-[0.15em] uppercase leading-tight ${
                      active ? 'text-[#F9F9F9]' : 'text-[#141313]/60 group-hover:text-[#141313]'
                    }`}>
                      {item.label}
                    </p>
                    <p className={`text-[8px] font-mono tracking-wider mt-0.5 ${
